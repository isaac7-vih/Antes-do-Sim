...
function proximaTela() {
  if (!ordemDasTelas || telaAtual >= ordemDasTelas.length - 1) return;

  const telaAtualEl = document.getElementById(ordemDasTelas[telaAtual]);
  if (telaAtualEl) telaAtualEl.classList.remove('ativa');

  setTimeout(() => {
    telaAtual++;
    const proximaTelaEl = document.getElementById(ordemDasTelas[telaAtual]);
    if (proximaTelaEl) proximaTelaEl.classList.add('ativa');
